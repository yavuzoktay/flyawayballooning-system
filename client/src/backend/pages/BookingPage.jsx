...
import { Container, FormControl, InputLabel, MenuItem, OutlinedInput, Select, Button, Dialog, DialogActions, DialogContent, DialogTitle, TextField, Grid, Typography, Box, Divider, IconButton, Table, TableHead, TableRow, TableCell, TableBody, TableContainer, FormControlLabel, Switch, FormGroup, Checkbox, Chip, InputAdornment, useTheme, useMediaQuery, Collapse } from "@mui/material";
...
const BookingPage = () => {
    ...
    const [mobileBookingFiltersOpen, setMobileBookingFiltersOpen] = useState(false);
    const [mobileVoucherFiltersOpen, setMobileVoucherFiltersOpen] = useState(false);
    ...
    const parseLocalNoTzDateTime = (value) => {
        ...
        // Matches timezone-less strings like "2026-04-02 17:30" / "2026-04-02 17:30:00"
        // (no trailing Z or offset).
    ...
    const handleSendEmail = async () => {
        ...
        if (sendMessageSmsChecked) {
            if (!isBulk) {
                // Get phone number from selectedBookingForEmail or bookingDetail
                // Priority: bookingDetail.booking > selectedBookingForEmail
                const bookingData = bookingDetail?.booking || selectedBookingForEmail || {};
                const phone = isVoucher 
                    ? (bookingData.purchaser_phone || bookingData.phone || bookingData.mobile || selectedBookingForEmail?.purchaser_phone || selectedBookingForEmail?.phone || selectedBookingForEmail?.mobile || '')
        ...
        try {
            ...
            if (sendMessageSmsChecked && !skipSingleSmsForNonUkNumber) {
                ...
                // Custom Message: use ONLY the personalized note as the full SMS body (match Send Message to Guests / Manifest)
                ...
                if (!smsMessageToSend || smsMessageToSend.trim().length === 0) {
                    ...
                    // Don't add to smsPromises if message is empty
                } else {
                    if (isBulk) {
                    if (isBulkVoucher) {
                    ...
                    // Get phone number from bookingDetail or selectedBookingForEmail
                    // Priority: bookingDetail.booking > selectedBookingForEmail
                    const bookingData = bookingDetail?.booking || selectedBookingForEmail || {};
                    const phone = isVoucher 
                        ? (bookingData.purchaser_phone || bookingData.phone || bookingData.mobile || selectedBookingForEmail?.purchaser_phone || selectedBookingForEmail?.phone || selectedBookingForEmail?.mobile || '')
                        ...
                        if (isVoucher) {
                            smsPromises.push(
                                axios.post('/api/sendVoucherSms', {
                                    ...
                                    body: smsMessageToSend,
                                    templateId: smsTemplateId || (smsForm.template && smsForm.template !== 'custom' ? smsForm.template : null),
                                    voucherData: dataForApi
                                })
                            );
                        } else {
                            smsPromises.push(
                                axios.post('/api/sendBookingSms', {
                                    bookingId: selectedBookingForEmail?.id || bookingDetail?.booking?.id,
                                    to: cleanedPhone,
                                    body: smsMessageToSend,
                                    templateId: smsTemplateId || (smsForm.template && smsForm.template !== 'custom' ? smsForm.template : null),
                                    bookingData: dataForApi
                                })
                            );
                        }
                    } else {
...
setBookingDetail(finalVoucherDetail);
    ...
    const handleGiftVoucherEmail = () => {
        ...
        const fauxBooking = {
            ...
            purchaser_name: voucher.purchaser_name || voucher.name || bookingDetail?.booking?.name || '',
            purchaser_email: purchaserEmail,
            purchaser_phone: voucher.purchaser_phone || voucher.phone || bookingDetail?.booking?.phone || '',
            recipient_name: voucher.recipient_name || '',
            recipient_email: voucher.recipient_email || '',
    ...
    const handleSendSms = async () => {
        ...
        const hasSmsBody = isCustom
            ? (smsPersonalNote && smsPersonalNote.trim().length > 0)
            : (smsForm.message && smsForm.message.trim().length > 0);
        if (!hasSmsBody) {
            alert(isCustom ? 'Please enter your custom message in the personalized note field.' : 'Please fill message');
            return;
        }
        ...
        // Custom Message: use ONLY smsPersonalNote. Template: use template + optional note
        const finalMessage = isCustom
            ? (smsPersonalNote && smsPersonalNote.trim()) ? smsPersonalNote.trim() : ''
            : (smsPersonalNote ? `${smsForm.message}${smsForm.message ? '\n\n' : ''}${smsPersonalNote}` : smsForm.message);
                                ...
                                <div className="booking-top-wrap">
                                    ...
                                    <div className="booking-filter-heading" style={{
                                        ...
                                        alignItems: 'center', 
                                            ...
                                            <OutlinedInput
                                                readOnly
                                                onClick={() => setMobileBookingFiltersOpen(prev => !prev)}
                                                value=""
                                                sx={{
                                                    cursor: 'pointer',
                                                    height: '32px',
                                                ...
                                                startAdornment={
                                                    <InputAdornment position="start" sx={{ margin: 0, position: 'relative' }}>
                                                        <FilterListIcon fontSize="small" color={mobileBookingFiltersOpen || filters.experience || filters.status || filters.voucherType || filters.location ? 'primary' : 'secondary'} />
                                                        {(filters.experience || filters.status || filters.voucherType || filters.location) && (
                                    ...
                                    {isMobile ? (
                                        <Collapse in={mobileBookingFiltersOpen} timeout="auto" unmountOnExit>
                                            <Box sx={{
                                                mt: 1,
                                                p: 1.5,
                                                bgcolor: '#f8f9fa',
                                                ...
                                                <div className="booking-filter-field" style={{ flex: '1 1 calc(50% - 2px)', minWidth: 0, margin: '0' }}>
                                                    <FormControl sx={{ m: 0.5, minWidth: 0, width: '100%' }} size="small">
                                                        <InputLabel id="book-flight-type-label-m" sx={{ fontSize: '11px' }}>Experience</InputLabel>
                                                        <Select
                                                            labelId="book-flight-type-label-m"
                                                        ...
                                                        >
                                                            ...
                                                            <MenuItem value="Private Charter" sx={{ fontSize: '11px' }}>Private Charter</MenuItem>
                                                            <MenuItem value="Shared Flight" sx={{ fontSize: '11px' }}>Shared Flight</MenuItem>
                                                        </Select>
                                                    </FormControl>
                                                </div>
                                                <div className="booking-filter-field" style={{ flex: '1 1 calc(50% - 2px)', minWidth: 0, margin: '0' }}>
                                                    <FormControl sx={{ m: 0.5, minWidth: 0, width: '100%' }} size="small">
                                                        <InputLabel id="book-Select-label-m" sx={{ fontSize: '11px' }}>Status</InputLabel>
                                                        <Select
                                                            labelId="book-Select-label-m"
                                                        ...
                                                        >
                                                            ...
                                                            <MenuItem value="Cancelled" sx={{ fontSize: '11px' }}>Cancelled</MenuItem>
                                                        </Select>
                                                    </FormControl>
                                                </div>
                                                <div className="booking-filter-field" style={{ flex: '1 1 calc(50% - 2px)', minWidth: 0, margin: '0' }}>
                                                    <FormControl sx={{ m: 0.5, minWidth: 0, width: '100%' }} size="small">
                                                        <InputLabel id="book-voucher-type-label-m" sx={{ fontSize: '11px' }}>Voucher Type</InputLabel>
                                                        <Select
                                                            labelId="book-voucher-type-label-m"
                                                ...
                                                <div className="booking-filter-field" style={{ flex: '1 1 calc(50% - 2px)', minWidth: 0, margin: '0' }}>
                                                    <FormControl sx={{ m: 0.5, minWidth: 0, width: '100%' }} size="small">
                                                        <InputLabel id="book-location-label-m" sx={{ fontSize: '11px' }}>Location</InputLabel>
                                                        <Select
                                                            labelId="book-location-label-m"
                                                ...
                                                {(filters.experience || filters.status || filters.voucherType || filters.location) && (
                                                    <Box sx={{ width: '100%', mt: 0.5, textAlign: 'center' }}>
                                                        <Button size="small" onClick={() => { handleFilterChange("experience", ""); handleFilterChange("status", ""); handleFilterChange("voucherType", ""); handleFilterChange("location", ""); }} sx={{ fontSize: '11px', textTransform: 'none' }}>
                                                            Clear Filters
                                                        </Button>
                                                    </Box>
                                                )}
                                ...
                                <div className="booking-top-wrap">
                                    ...
                                    <div className="booking-filter-heading" style={{
                                        ...
                                        alignItems: 'center', 
                                            ...
                                            <OutlinedInput
                                                readOnly
                                                onClick={() => setMobileVoucherFiltersOpen(prev => !prev)}
                                                value=""
                                                sx={{
                                                    cursor: 'pointer',
                                                    height: '32px',
                                                ...
                                                startAdornment={
                                                    <InputAdornment position="start" sx={{ margin: 0, position: 'relative' }}>
                                                        <FilterListIcon fontSize="small" color={mobileVoucherFiltersOpen || filters.voucherType || filters.experience || filters.actualVoucherType || filters.redeemedStatus ? 'primary' : 'secondary'} />
                                                        {(filters.voucherType || filters.experience || filters.actualVoucherType || filters.redeemedStatus) && (
                                                            <Box sx={{ position: 'absolute', top: -4, right: -6, width: 8, height: 8, borderRadius: '50%', bgcolor: 'primary.main' }} />
                                                        )}
                                                    </InputAdornment>
                                                }
                                    ...
                                    {isMobile ? (
                                        <Collapse in={mobileVoucherFiltersOpen} timeout="auto" unmountOnExit>
                                            <Box sx={{
                                                mt: 1,
                                                p: 1.5,
                                                bgcolor: '#f8f9fa',
                                                ...
                                                <div className="booking-filter-field" style={{ flex: '1 1 calc(50% - 2px)', minWidth: 0, margin: '0' }}>
                                                    <FormControl sx={{ m: 0.5, minWidth: 0, width: '100%' }} size="small">
                                                        <InputLabel id="book-voucher-type-label-vm" sx={{ fontSize: '11px' }}>Book Flight</InputLabel>
                                                        <Select
                                                            labelId="book-voucher-type-label-vm"
                                                            value={filters.voucherType}
                                                            onChange={(e) => handleFilterChange("voucherType", e.target.value)}
                                                            label="Book Flight"
                                                            sx={{ fontSize: '11px', height: '32px', '& .MuiSelect-select': { padding: '6px 8px', fontSize: '11px' } }}
                                                        >
                                                            <MenuItem value="" sx={{ fontSize: '11px' }}><em>Select</em></MenuItem>
                                                            <MenuItem value="Flight Voucher" sx={{ fontSize: '11px' }}>Flight Voucher</MenuItem>
                                                            <MenuItem value="Gift Voucher" sx={{ fontSize: '11px' }}>Gift Voucher</MenuItem>
                                                        </Select>
                                                    </FormControl>
                                                </div>
                                                <div className="booking-filter-field" style={{ flex: '1 1 calc(50% - 2px)', minWidth: 0, margin: '0' }}>
                                                    <FormControl sx={{ m: 0.5, minWidth: 0, width: '100%' }} size="small">
                                                        <InputLabel id="book-flight-type-label-vm" sx={{ fontSize: '11px' }}>Experience</InputLabel>
                                                        <Select
                                                            labelId="book-flight-type-label-vm"
                                                            value={filters.experience}
                                                            label="Experience"
                                                            onChange={(e) => handleFilterChange("experience", e.target.value)}
                                                            sx={{ fontSize: '11px', height: '32px', '& .MuiSelect-select': { padding: '6px 8px', fontSize: '11px' } }}
                                                ...
                                                <div className="booking-filter-field" style={{ flex: '1 1 calc(50% - 2px)', minWidth: 0, margin: '0' }}>
                                                    <FormControl sx={{ m: 0.5, minWidth: 0, width: '100%' }} size="small">
                                                        <InputLabel id="book-actual-voucher-type-label-vm" sx={{ fontSize: '11px' }}>Voucher Type</InputLabel>
                                                        <Select
                                                            labelId="book-actual-voucher-type-label-vm"
                                                ...
                                                {(filters.voucherType || filters.experience || filters.actualVoucherType || filters.redeemedStatus) && (
                                                    <Box sx={{ width: '100%', mt: 0.5, textAlign: 'center' }}>
                                                        <Button size="small" onClick={() => { handleFilterChange("voucherType", ""); handleFilterChange("experience", ""); handleFilterChange("actualVoucherType", ""); handleFilterChange("redeemedStatus", ""); }} sx={{ fontSize: '11px', textTransform: 'none' }}>
                                                            Clear Filters
                                                        </Button>
                                                    </Box>
                                                )}
                                                        ...
                                                        <TableBody>
                                                            {historyRows.map((h, i) => (
                                                                <TableRow key={i}>
                                                                    ...
                                                                    }}>{h.flight_date ? formatHistoryDateDisplay(h.flight_date) : (h.changed_at ? formatHistoryDateDisplay(h.changed_at) : '-')}</TableCell>
                                                                    <TableCell sx={{ 
                                                                        fontSize: isMobile ? '11px' : 'inherit',
                                                                        padding: isMobile ? '6px 4px' : '16px'
                ...
                >
                    <DialogTitle sx={{ 
                        ...
                        <Box component="span" sx={{ 
                            fontWeight: 700, 
                            fontSize: isMobile ? '1.125rem' : '20px',
                            color: isMobile ? 'inherit' : '#1f2937'
                        }}>
                            Payments / Promos
                        </Box>
                    ...
                    <DialogContent dividers sx={{ background: '#ffffff', p: 0, padding: isMobile ? '0' : '0' }}>
                        {paymentHistoryLoading ? (
                            <Box sx={{ p: isMobile ? 2 : 3, textAlign: 'center' }}>
                                <Typography variant="body2" sx={{ fontSize: isMobile ? 14 : '14px' }}>Loading payment history...</Typography>
                            </Box>
                            ...
                            <Box>
                                ...
                                <Box sx={{ 
                                    ...
                                    px: isMobile ? 1.5 : 3, 
                                    py: isMobile ? 1 : 1.5, 
                                    borderBottom: isMobile ? '1px solid #e2e8f0' : '1px solid #e5e7eb',
                                    background: '#f8f9fa'
                                }}>
                                    <Box sx={{ 
                                        flex: '0 0 200px', 
                                        ...
                                        <Box key={payment.id || index} sx={{ borderBottom: '1px solid #e2e8f0' }}>
                                                ...
                                                <Box sx={{ 
                                                    ...
                                                    {payment.payment_status === 'refunded' || payment.origin === 'refund' ? (
                                                        <Typography variant="body2" sx={{ 
                                                            fontWeight: 600,
                                                            textTransform: 'uppercase',
                                                            fontSize: isMobile ? '0.65rem' : '12px',
                                                            px: isMobile ? 0.5 : 1,
                                                            py: isMobile ? 0.25 : 0.5,
                                            ...
                                            {isExpanded && (
                                                <Box sx={{ 
                                                    ...
                                                    py: isMobile ? 1.5 : 2, 
                                                    background: '#ffffff',
                                                    borderTop: isMobile ? '1px solid #e2e8f0' : '1px solid #e5e7eb'
                                                }}>
                ...
                >
                                            ...
                                            <Box sx={{
                                                ...
                                                <Box sx={{
                                                    ...
                                                    {(() => {
                                                        ...
                                                        const finalMessage = trimmedNote
                                                            ? `${messageWithPrompts}${messageWithPrompts ? '\n\n' : ''}${trimmedNote}`
                                                            : messageWithPrompts;
                    ...
                    <DialogActions sx={{ 
                        ...
                        <Button 
                            ...
                            disabled={smsSending || !smsForm.to || (() => {
                                const isCustom = !smsForm.template || smsForm.template === 'custom';
                                return isCustom ? !(smsPersonalNote && smsPersonalNote.trim()) : !smsForm.message;
                            })()}
                        >
