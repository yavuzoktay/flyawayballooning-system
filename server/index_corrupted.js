...
// Sync activity pricing to private charter voucher types
app.post('/api/sync-activity-pricing', (req, res) => {
    ...
    // Get activity data
    const activitySql = `SELECT location, private_charter_pricing FROM activity WHERE id = ?`;
    con.query(activitySql, [activity_id], (activityErr, activityResult) => {
        if (activityErr) {
            console.error('Error fetching activity:', activityErr);
            return res.status(500).json({ success: false, message: 'Database error', error: activityErr.message });
        }
        ...
        try {
            if (activity.private_charter_pricing) {
                pricingData = JSON.parse(activity.private_charter_pricing);
            }
        } catch (e) {
            console.error('Error parsing activity pricing:', e);
            return res.status(500).json({ success: false, message: 'Invalid pricing data format' });
        }
        
        if (Object.keys(pricingData).length === 0) {
            return res.json({ success: true, message: 'No pricing data to sync' });
        }
        
        // Get all private charter voucher types
        const voucherTypesSql = `SELECT id, title FROM private_charter_voucher_types WHERE is_active = 1`;
        con.query(voucherTypesSql, [], (voucherTypesErr, voucherTypesResult) => {
            if (voucherTypesErr) {
                console.error('Error fetching voucher types:', voucherTypesErr);
                return res.status(500).json({ success: false, message: 'Database error', error: voucherTypesErr.message });
            }
            
            let updatedCount = 0;
            let errors = [];
            
            // Update each voucher type with its pricing
            voucherTypesResult.forEach(vt => {
                if (pricingData[vt.title]) {
                    const updateSql = `UPDATE private_charter_voucher_types SET price_per_person = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?`;
                    con.query(updateSql, [pricingData[vt.title], vt.id], (updateErr) => {
                        if (updateErr) {
                            console.error(`Error updating voucher type ${vt.title}:`, updateErr);
                            errors.push(`Failed to update ${vt.title}: ${updateErr.message}`);
                        } else {
                            updatedCount++;
                        }
                    });
            ...
            setTimeout(() => {
                if (errors.length > 0) {
                    res.json({ 
                        success: true, 
                        message: `Synced ${updatedCount} voucher types with ${errors.length} errors`, 
                        updatedCount, 
                        errors 
                    });
                } else {
                    res.json({ 
                        success: true, 
                        message: `Successfully synced ${updatedCount} voucher types`, 
                        updatedCount 
                    });
                }
            }, 1000);
...
app.get('/api/private-charter-voucher-types', (req, res) => {
    ...
    const location = req.query.location; // Get location parameter if provided
    ...
    con.query(sql, params, (err, result) => {
        ...
        // If location is provided, get activity-specific pricing
        if (location && result.length > 0) {
            const activitySql = `SELECT private_charter_pricing FROM activity WHERE location = ? AND status = 'Live' ORDER BY id ASC LIMIT 1`;
            con.query(activitySql, [location], (activityErr, activityResult) => {
                if (activityErr) {
                    console.error('Error fetching activity pricing:', activityErr);
                    // Return voucher types without pricing if there's an error
                    res.json({ success: true, data: result });
                    return;
                }
                
                let activityPricing = {};
                if (activityResult && activityResult.length > 0 && activityResult[0].private_charter_pricing) {
                    try {
                        activityPricing = JSON.parse(activityResult[0].private_charter_pricing);
                    } catch (e) {
                        console.error('Error parsing activity pricing:', e);
                        activityPricing = {};
                    }
                ...
                // Enhance voucher types with activity-specific pricing
                const enhancedResult = result.map(vt => {
                    const enhancedVt = { ...vt };
                    if (activityPricing[vt.title]) {
                        enhancedVt.price_per_person = activityPricing[vt.title].toString();
                        enhancedVt.price_unit = 'pp';
                    }
                    return enhancedVt;
                });
                
                console.log('Enhanced voucher types with activity pricing:', enhancedResult);
                res.json({ success: true, data: enhancedResult });
            });
        } else {
            // Return voucher types without pricing enhancement
            res.json({ success: true, data: result });
        }
    });
});
...
app.put('/api/private-charter-voucher-types/:id', experiencesUpload.single('private_charter_voucher_type_image'), (req, res) => {
    ...
    const sql = `
        UPDATE private_charter_voucher_types SET 
            title = ?, description = ?, image_url = ?, max_passengers = ?, 
            validity_months = ?, flight_days = ?, flight_time = ?, features = ?, 
            terms = ?, sort_order = ?, is_active = ?, price_per_person = ?, price_unit = ?
        WHERE id = ?
    `;
...
app.post('/api/addPassenger', (req, res) => {
    ...
    con.query(sql, values, (err, result) => {
        ...
        con.query(updatePaxSql, [booking_id, booking_id], (err2) => {
            ...
            // Now update the activity availability for this booking
            updateAvailabilityForBooking(booking_id, (updateErr) => {
                if (updateErr) {
                    console.error('Error updating availability after adding passenger:', updateErr);
                    // Still return success for passenger creation
                    return res.status(201).json({ success: true, passengerId: result.insertId, paxUpdated: true, availabilityUpdated: false });
                }
                res.status(201).json({ success: true, passengerId: result.insertId, paxUpdated: true, availabilityUpdated: true });
            });
        });
    });
});
    ...
    con.query(deletePassengerSql, [passenger_id, booking_id], (err, result) => {
        ...
        con.query(updatePaxSql, [booking_id, booking_id], (err2) => {
            if (err2) {
                ...
                updateAvailabilityForBooking(booking_id, (updateErr) => {
                    if (updateErr) {
                        console.error('Error updating availability after deleting passenger:', updateErr);
                        // Still return success for passenger deletion
                        return res.status(200).json({ success: true, message: 'Passenger deleted, pax updated, but availability update failed', paxUpdated: true, availabilityUpdated: false });
                    }
                    
                    // Get the current pax count
                    ...
                    con.query(checkPaxSql, [booking_id], (err3, rows) => {
                        if (err3) {
                            ...
                            // Still return success for passenger deletion
                            return res.status(200).json({ success: true, message: 'Passenger deleted, pax updated, availability updated, but pax check failed', paxUpdated: true, availabilityUpdated: true, remainingPax: null });
                        }
...
app.post('/api/deletePassenger', (req, res) => {
    ...
    con.query(deletePassengerSql, [passenger_id, booking_id], (err, result) => {
        ...
        con.query(updatePaxSql, [booking_id, booking_id], (err2) => {
            ...
            updateAvailabilityForBooking(booking_id, (updateErr) => {
                if (updateErr) {
                    console.error('Error updating availability after deleting passenger (POST):', updateErr);
                    // Still return success for passenger deletion
                    return res.status(200).json({ success: true, message: 'Passenger deleted, pax updated, but availability update failed', paxUpdated: true, availabilityUpdated: false });
                }
...
app.get('/api/activity/:id/availabilities', (req, res) => {
    ...
    // Fixed query to count actual passengers from passenger table, not just booking records
    const optimizedSql = `
        SELECT 
            aa.*,
            a.location,
            a.flight_type,
            COALESCE(passenger_counts.total_passengers, 0) as total_booked,
            CASE 
                WHEN COALESCE(passenger_counts.total_passengers, 0) >= aa.capacity THEN 'Closed'
                ELSE aa.status
            END as calculated_status,
            GREATEST(0, aa.capacity - COALESCE(passenger_counts.total_passengers, 0)) as calculated_available
        ...
        LEFT JOIN (
            ...
            LEFT JOIN (
                ...
                FROM passenger 
                GROUP BY booking_id
            ) p ON ab.id = p.booking_id
            WHERE DATE(ab.flight_date) >= CURDATE() - INTERVAL 30 DAY
            AND ab.activity_id = ?
            GROUP BY DATE(ab.flight_date), TIME(ab.flight_date)
        ) as passenger_counts ON DATE(aa.date) = passenger_counts.flight_date AND TIME(aa.time) = passenger_counts.flight_time
...
app.post('/api/activity/:id/updateAvailabilityStatus', (req, res) => {
    ...
    const sql = `
        ...
        ) as passenger_counts ON 
            DATE(aa.date) = passenger_counts.flight_date AND
            TIME(aa.time) = passenger_counts.time_slot AND
            EXISTS (
                SELECT 1 FROM activity a 
                WHERE a.id = aa.activity_id 
                AND a.location = passenger_counts.location
            )
        SET 
            aa.status = CASE 
                WHEN passenger_counts.total_passengers >= aa.capacity THEN 'Closed'
                ELSE 'Open'
            END,
            aa.available = GREATEST(0, aa.capacity - passenger_counts.total_passengers)
        WHERE aa.activity_id = ?
    `;
...
// Function to update availability for a specific booking
const updateAvailabilityForBooking = (bookingId, callback) => {
    // Get booking details to find the activity and time slot
    const getBookingSql = `
        SELECT ab.flight_date, ab.location, ab.activity_id, ab.time_slot, a.id as activity_id
        FROM all_booking ab
        LEFT JOIN activity a ON a.location = ab.location AND a.status = 'Live'
        WHERE ab.id = ?
    `;
    
    con.query(getBookingSql, [bookingId], (err, bookingResult) => {
        if (err) {
            console.error('Error getting booking details for availability update:', err);
            return callback(err);
        }
        ...
        if (!booking.activity_id) {
            console.log('No activity found for location:', booking.location);
            return callback(null); // No error, just no activity to update
        }
        ...
        if (booking.flight_date) {
            if (typeof booking.flight_date === 'string') {
                if (booking.flight_date.includes(' ')) {
                    const parts = booking.flight_date.split(' ');
                    bookingDate = parts[0];
                    bookingTime = parts[1];
                } else if (booking.flight_date.includes('T')) {
                    const dateObj = new Date(booking.flight_date);
                    bookingDate = dateObj.toISOString().split('T')[0];
                    bookingTime = dateObj.toTimeString().split(' ')[0];
                } else {
                    bookingDate = booking.flight_date;
                }
            } else if (booking.flight_date instanceof Date) {
                bookingDate = booking.flight_date.toISOString().split('T')[0];
                bookingTime = booking.flight_date.toTimeString().split(' ')[0];
            }
        }
        
        // Use time_slot if available
        if (booking.time_slot && !bookingTime) {
            bookingTime = booking.time_slot;
        }
        
        if (bookingDate && bookingTime) {
            console.log(`Updating availability for booking ${bookingId}: date=${bookingDate}, time=${bookingTime}, activity=${booking.activity_id}`);
            updateSpecificAvailability(bookingDate, bookingTime, booking.activity_id, 0); // 0 means recalculate from passenger count
        } else {
            console.log(`Cannot update availability for booking ${bookingId}: missing date or time`);
        }
...
const updateSpecificAvailability = async (date, time, activityId, passengerCount) => {
    try {
        ...
        con.query(checkSql, [date, time, activityId], (checkErr, checkResult) => {
            ...
            // If passengerCount is 0, recalculate from actual passenger count
            if (passengerCount === 0) {
                // Get actual passenger count for this time slot
                const getPassengerCountSql = `
                    SELECT SUM(p.passenger_count) as total_passengers
                    FROM all_booking ab 
                    LEFT JOIN (
                        SELECT 
                            booking_id, 
                    ...
                    AND TIME(ab.flight_date) = TIME(?)
                `;
                
                con.query(getPassengerCountSql, [activityId, date, time], (passengerErr, passengerResult) => {
                    if (passengerErr) {
                        console.error('Error getting passenger count for recalculation:', passengerErr);
                        return;
                    }
                    
                    const totalPassengers = passengerResult[0].total_passengers || 0;
                    console.log(`Recalculated passenger count for ${date} ${time}: ${totalPassengers}`);
                    
                    // Update availability based on actual passenger count
                    const newAvailable = Math.max(0, checkResult[0].capacity - totalPassengers);
                    const newStatus = totalPassengers >= checkResult[0].capacity ? 'Closed' : 'Open';
                    
                    const updateSql = `UPDATE activity_availability SET available = ?, status = ? WHERE date = ? AND time = ? AND activity_id = ?`;
                    con.query(updateSql, [newAvailable, newStatus, date, time, activityId], (err, result) => {
                        if (err) {
                            console.error('Error updating availability after recalculation:', err);
                        } else {
                            console.log(`Availability updated after recalculation: available=${newAvailable}, status=${newStatus}`);
                        }
                    });
                });
                return;
...
// Manual sync all availabilities to fix passenger count mismatches
app.post('/api/sync-all-availabilities', (req, res) => {
    console.log('Manual sync all availabilities started');
    
    // Get all activities
    const getActivitiesSql = `SELECT id, location FROM activity WHERE status = 'Live'`;
    con.query(getActivitiesSql, [], (err, activities) => {
        if (err) {
        ...
        activities.forEach((activity, index) => {
            setTimeout(() => {
                // Get all availabilities for this activity
                const getAvailabilitiesSql = `
                    SELECT aa.id, aa.date, aa.time, aa.capacity, aa.available, aa.status
                    FROM activity_availability aa 
                    WHERE aa.activity_id = ?
                `;
                
                con.query(getAvailabilitiesSql, [activity.id], (availErr, availabilities) => {
                    if (availErr) {
                        console.error(`Error fetching availabilities for activity ${activity.id}:`, availErr);
                        totalErrors++;
                        return;
                    }
                    
                    console.log(`Processing ${availabilities.length} availabilities for activity ${activity.id} (${activity.location})`);
                    
                    // Process each availability
                    availabilities.forEach((availability) => {
                        // Get actual passenger count for this availability
                        const getPassengerCountSql = `
                            SELECT SUM(p.passenger_count) as total_passengers
                            FROM all_booking ab 
                            LEFT JOIN (
                        ...
                        con.query(getPassengerCountSql, [activity.id, availability.date, availability.time], (passengerErr, passengerResult) => {
                            if (passengerErr) {
                                console.error(`Error getting passenger count for availability ${availability.id}:`, passengerErr);
                                totalErrors++;
                                return;
                            }
                            
                            const totalPassengers = passengerResult[0].total_passengers || 0;
                            const newAvailable = Math.max(0, availability.capacity - totalPassengers);
                            const newStatus = totalPassengers >= availability.capacity ? 'Closed' : 'Open';
                            ...
                            if (newAvailable !== availability.available || newStatus !== availability.status) {
                                ...
                                con.query(updateSql, [newAvailable, newStatus, availability.id], (updateErr) => {
                                    if (updateErr) {
                                        console.error(`Error updating availability ${availability.id}:`, updateErr);
                                        totalErrors++;
                                    } else {
                                        console.log(`Updated availability ${availability.id}: available=${newAvailable}, status=${newStatus}, total_passengers=${totalPassengers}`);
                                        totalUpdated++;
                                    }
                                });
                            }
            ...
            }, index * 1000); // Process one activity per second to avoid overwhelming the database
        });
        
        // Wait for all processing to complete
        setTimeout(() => {
            console.log(`Manual sync completed: ${totalUpdated} availabilities updated, ${totalErrors} errors`);
            res.json({
                success: true,
                message: `Manual sync completed: ${totalUpdated} availabilities updated, ${totalErrors} errors`,
                totalUpdated,
                totalErrors
            });
        }, (activities.length + 5) * 1000); // Wait for all activities to process + 5 seconds buffer
    });
});
    ...
    con.query(getAvailabilitiesSql, [id], (err, availabilities) => {
        ...
        const updatePromises = availabilities.map(availability => {
            return new Promise((resolve) => {
                ...
                con.query(getPassengerCountSql, [id, availability.date, availability.location, availability.time], (passengerErr, passengerResult) => {
                    if (passengerErr) {
                        console.error('Error getting passenger count:', passengerErr);
                        resolve(false);
                        return;
                    }
